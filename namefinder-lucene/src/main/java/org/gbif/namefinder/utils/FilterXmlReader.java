...
/**
 * A simple FilterReader that strips XML/HTML tags (or anything between pairs of
 * angle brackets) out of a stream of characters.
 */
public class FilterXmlReader extends FilterReader {

  public FilterXmlReader(Reader in) {
    super(in);
  }
   ...
   * HTML tags and replaces them with a space. (in is a protected field of the superclass).
   */
    ...
    while (numchars == 0) {
      ...
      for (int i = from; i < from + numchars; i++) {
        if (!intag) { // If not in an HTML tag
          if (buf[i] == '<'){
            buf[last++] = ' '; // replaces tag with space
